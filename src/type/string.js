...
// copied from removed dependency 'validator'
const isAlphanumericRe = /^[0-9A-Z]+$/i
const isEmailRe = /^[^\.\s@:](?:[^\s@:]*[^\s@:\.])?@[^\.\s@]+(?:\.[^\.\s@]+)*$/
const isUUIDV4Re = /^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i
const isLowercase = str => (
  typeof str === 'string' && str === str.toLowerCase())
const isUppercase = str => (
  typeof str === 'string' && str === str.toUpperCase())
...
/**
x * Set a custom validator that will be called with the string. The validator
