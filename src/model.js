...
Model.prototype._promisesReady = function() {
  ...
  var verifyAll = function() {
    ...
    .then(function() {
      ...
      for (i=0; i<self._pendingPromisesStates.length; i++) {
        if (!self._pendingPromisesStates[i]) {
          allFullfilled = false;
          break;
        }
      }
      ...
      // REMOVED:
      // es6 promises do not support bluebird promise 'isFulfilled'
      //
...
Model.prototype._waitFor = function(promise) {
  ...
  const pendingPromiseStateIndex = +this._pendingPromisesStates.length
  this._pendingPromisesStates.push(false)
  this._pendingPromises.push(promise);

  promise.then(() => {
    self._pendingPromisesStates[pendingPromiseStateIndex] = true
  })
...
Model.prototype._createIndex = function(name, fn, opts) {
  ...
  // rethinkdb-ts requires no opts or valid opts
...
/* legacy, removed because: side-effects, bad rethinkdbdash hard-dependency
(function() {
